from flask import Flask, render_template, request, make_response, redirect, abort, session, redirect, url_for, abort
import oauthlib
import os
...
def delete_post(post_id):
    ...
    if post:
        ...
        return "No se encontró el post con el ID especificado."
    
def delete_comment(comment_id): 
    # Buscar el comentario en la base de datos
    comment = db.comentarios.find_one({"_id": ObjectId(comment_id)})
    if comment:
        # Eliminar el comentario de la base de datos
        db.comentarios.delete_one({"_id": ObjectId(comment_id)})
        return redirect('/archivos')
    else:
...
def delete_post_route(post_id):
    return delete_post(post_id) # Llama a la función delete_post de funciones.py
@app.route('/delete_comment/<comment_id>', methods=['POST'])
def delete_comment_route(comment_id):
    return delete_comment(comment_id) # Llama a la función delete_comment de funciones.py
